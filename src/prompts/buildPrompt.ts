...
const dialectHints: Record<SqlDialect, string> = {
...
export function buildSystemPrompt(dialect: SqlDialect, platformSystemPrompt?: string): string {
  const base = [
  ...
  const blocks = payload.tables.map((t, i) => {
    ...
    if (t.sourceKind === "event") {
      const eventLines = (t.events ?? [])
        .map((ev) => {
          const attrs =
            ev.attributes?.map((a) => `${a.name}${a.comment ? `: ${a.comment}` : ""}`).join(", ") || "(无属性)";
